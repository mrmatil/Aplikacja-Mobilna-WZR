...
//    }
    ...
    private func parseData(data:String){
        let tempSplittedData = data.components(separatedBy: "class=\"pracownik_b\"")
        var splittedData = [String]()
        
        for temp in tempSplittedData{
            splittedData.append("<div> <div class=\"pracownik_b\"" + temp)
        }
        
        var dataArray = [LecturersArray]()
        
        for onePerson in 1...splittedData.count-2{
            
            do {
                                
                let doc = try SwiftSoup.parse(splittedData[onePerson])
                                
                let name = try doc.select(".pracownik_b").text()
                let email = try doc.select(".wizytowka_info .plink").text()
                let info = try doc.select(".konsultacje").text()
                let url = try doc.select("a").attr("href")
                
                dataArray.append(LecturersArray(name: LecturersUtil.getTrueName(name: name),
                                                email: email,
                                                info: info,
                                                website: LecturersUtil.getUrl(url: url)))
